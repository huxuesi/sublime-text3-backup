    ...
    <div class="z-honsenInformation">
        ...
        <Tabs class="body" :value="curTag">
            <TabPane label="用户指引" name="guide">
                <div v-if="index >= curguidsidx && index < curguidsidx + 3" v-for="(post,index) in guides" :key="index" @click="toPost(post.pid)" class="item">
                    <img class="img" :src="`/static/img/honseninfo${index % 11}.png`">
            ...
            <TabPane label="精品知识" name="bestPost">
                <div v-if="index >= curbestPostsidx && index < curbestPostsidx + 3" v-for="(post,index) in bestPosts" :key="index" @click="toPost(post.pid)" class="item">
                    <img class="img" :src="`/static/img/honseninfo${(index % 10 + 11)}.png`">
                    <h3 class="tit single_ellipsis" :title="post.title">{{post.title}}</h3>
                    <div class="summary" v-html="post.summary"></div>
                </div>
            </TabPane>
    ...
    methods: {
        ...
        changecuridx(flag) {
            if( !flag ){
                ...
                if( this.curTag == 'guide' ){
                    this.curguidsidx = this.curguidsidx + 1 >= this.guides.length - 1 ? this.guides.length - 1 : this.curguidsidx + 1;
                }
                if( this.curTag == 'bestPost' ){
                    this.curbestPostsidx = this.curbestPostsidx + 1 >= this.bestPosts.length - 1 ? this.bestPosts.length - 1 : this.curbestPostsidx + 1;
                }
            }
        }
    },
