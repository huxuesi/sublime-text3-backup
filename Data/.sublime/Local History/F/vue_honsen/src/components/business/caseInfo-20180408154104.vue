...
<template>
    <div class="case_infov2">
        ...
        <div class="caseinfoformwrap">
            ...
            <div class="formitem2 contact"><span class="itemtit">联系人：</span>
                <span v-if="index < 3" v-for="(cts, index) in contacts" class="commontagwrapspanv2">
                    <Tag :closable="!cts.isCustomerContacts" @on-close="toSubmitAuth(casecontacts.filter(e => e.id != cts.id), casecontacts.filter(e => e.id != cts.id).map(e => e.id), false, 'contact')" :class="{gray:  cts.isCustomerContacts && !cts.isAuth}" class="commontagwrapv2">{{cts.name}}</Tag>
                ...
                <Poptip
                    ...
                    <div slot="content">
                        <div class="customer_avatar_item">
                            <span v-if="index > 2" v-for="(cts, index) in contacts" class="commontagwrapspanv2">
                                <Tag :closable="!cts.isCustomerContacts" @on-close="toSubmitAuth(casecontacts.filter(e => e.id != cts.id), casecontacts.filter(e => e.id != cts.id).map(e => e.id), false, 'contact')" :class="{gray:  cts.isCustomerContacts && !cts.isAuth}" class="commontagwrapv2">{{cts.name}}</Tag>
                                <div @click="addCustomer(cts, 'contact')" v-if="cts.isCustomerContacts && !cts.isAuth" class="commontagwrapmaskv2">
                                    +
                                </div>
                ...
                <Icon @click.native="openAddmember($event, 'contact', '联系人', casecontacts, false, true)" type="android-add-circle" :class="{hover: 0}" class="addnewiconv2"></Icon>
            </div>
            ...
            <div class="formitem2 xiezuozhe"><span class="itemtit">协作者：</span>
                <Tag v-if="index < 3" v-for="(cct, index) in coactors" closable @on-close="removeCustomer(cct.id, 'coactor')" :key="cct.cauid"  class="commontagwrapv2">{{cct.name}}</Tag>
                ...
                <Poptip
                    ...
                    <div slot="content">
                        <div class="customer_avatar_item">
                            <Tag v-if="index > 2" v-for="(cct, index) in coactors" closable @on-close="removeCustomer(cct.id, 'coactor')" :key="cct.ctmctctid"  class="commontagwrapv2">{{cct.name}}</Tag>
                        </div>
                    </div>
                </Poptip>

                <Icon @click.native="openAddmember($event, 'coactor', '协作者', coactors, true, false)" type="android-add-circle" :class="{hover: 0}" class="addnewiconv2"></Icon>
            </div>
            ...
            <div class="formitem2 outvisiter"><span class="itemtit">外访者：</span>
                <Tag v-if="index < 3" v-for="(vst, index) in visitors" closable @on-close="removeCustomer(vst.id, 'visitor')" :key="vst.cauid"  class="commontagwrapv2">{{vst.name}}</Tag>
                ...
                <Poptip
                    ...
                    <div slot="content">
                        <div class="customer_avatar_item">
                            <Tag v-if="index > 2" v-for="(vst, index) in visitors" closable @on-close="removeCustomer(vst.id, 'visitor')" :key="vst.ctmctctid"  class="commontagwrapv2">{{vst.name}}</Tag>
                        </div>
                    </div>
                </Poptip>

                <Icon @click.native="openAddmember($event, 'visitor', '外访者', visitors, true, false)" type="android-add-circle" :class="{hover: 0}" class="addnewiconv2"></Icon>
            </div>
        ...
        <Modal
            ...
            <div class="modal_tit">案件基本信息
                <div class="icon-can-dacha" @click.stop="caseEditModal = false">
                    <Icon type="android-close"></Icon>
                </div>
            </div>
            <Form :model="caseInfoModal" :rules="ruleValidate" ref="case_edit_form" :label-width="130" class="commonformv2 case_edit_form">
...
export default {
    ...
    methods: {
        ...
        //     this.addMemberModal = true;
        // },
        toSubmitAuth(users,ids,isNoChange,typestr) {
            if (isNoChange) {
                return;
            }
            let type;
            ...
            this.$http.post(this.$apiConfig.bizGroup.setCaseAuth.replace(/\{ctmid\}/, this.caseInfo.ctmid).replace(/\{caseid\}/, this.caseInfo.caseid), {uids: ids, type: type}).then(response => {
                ...
                if( data.retcode == RETCODE_OK ){
                    ...
                    this.$emit('changeRelateUser', users, type, 'submit');
                }else{
                    let text;
        ...
        addCustomer(contact, type){
            let uids = [contact.id];
            this.casecontacts.forEach((item) => {
                uids.push(item.id);
            });
            let param = {
                uids: uids,
        ...
        removeCustomer(uid, type) {
            ...
            this.$ajax('post', `/biz/case/${this.caseInfo.ctmid}/assoc/${this.caseInfo.caseid}/del`, param, data => {
                ...
                this.$emit('changeRelateUser', uid, type, 'remove');
        ...
        //     });
        // },
        openAddmember(e, name, type, users, onlyLawyer, noLawyer) {
