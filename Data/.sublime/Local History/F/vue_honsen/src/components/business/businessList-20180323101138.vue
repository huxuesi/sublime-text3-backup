...
export default {
    ...
    methods: {
        ...
        // },
        getCaseList($state) {
            let params = { page: this.caselist.arr.length / this.caselist.perpage + 1, perpage: this.caselist.perpage};
            params = Object.assign(params, this.addparams);
            this.$ajax('get', this.$apiConfig.bizGroup.getBizList, params, 'caselist.loading', (data) => {
                let casearr = data.data.data;
                this.caselist.total = data.data.total;
                this.caselist.arr = this.caselist.arr.concat(casearr.map(val => { return val;}));
                if( casearr.length < this.caselist.perpage ){
                    $state.complete();
                }else{
                    $state.loaded();
                }
