    ...
    <div class="caseeditv2">
        ...
        <div v-if="curflowobj.casefid" class="caseeditmiddlewrap">
            ...
            <div v-if="bottomcasekeepnav" class="middleconnonnavwrap floatpanel">
                <caseFlowKeep :caseInfo="caseInfo" :curflow="curflowobj"></caseFlowKeep>
            </div>
...
export default {
    name: 'caseEdit',
    components: { editor, comment, caseInfo, caseFlowKeep},
    data() {
        return {
            imgUrl,
            caseId: null,
    ...
    computed: {
        flowUpParam() {
            return {
                about_id: this.curflowobj.casefid,
    ...
    methods: {
        ...
        canEditOwnCreated(createrid) {
            return !this.caseInfo.status && (this.oRole.isOwner || ((this.oRole.isTeam || this.oRole.isCoactor) && (createrid == this.$store.state.mineInfo.id)));
        },
        flowUpSuccess(response, file, fileList) {
            if (response.retcode == RETCODE_OK) {
                ...
                this.$ajax('post', `/biz/flow/${this.caseInfo.caseid}/store/${this.curflowobj.casefid}`, {filesjson: jsonString}, data => {
                    this.curflowobj.filesjson.push(newFileObj);
