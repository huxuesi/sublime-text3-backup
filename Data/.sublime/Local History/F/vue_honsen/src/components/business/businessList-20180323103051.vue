    ...
    <div class="businesslistwrapv3">
        ...
        <div class="bizlistwrapv3">
            ...
            <div v-else class="cuswrap">
                ...
                <div class="bizlistbody">
                    <div v-for="cuslist in customerlist" class="bizlistbodyitem">
...
export default {
    ...
    methods: {
        ...
        //     });
        // },
        getCaseList($state, list) {
            let params = { page: list.arr.length / list.perpage + 1, perpage: list.perpage};
            params = Object.assign(params, this.addparams);
            console.log(list)
            this.$ajax('get', this.$apiConfig.bizGroup.getBizList, params, list.objname + '.loading', (data) => {
                let casearr = data.data.data;
                list.total = data.data.total;
                list.arr = list.arr.concat(casearr.map(val => { return val;}));
                if( casearr.length < list.perpage ){
                    $state.complete();
                }else{
                    $state.loaded();
                }
        ...
        onInfiniteLoading($state) {
            if( this.curBigBigNav == 'biz' ){
                this.getCaseList($state, this.caselist);
            }
            if( this.curBigBigNav == 'cus' ){
                this.getCaseList($state, this.customerlist);
            }
        }
    },
    filters: {
