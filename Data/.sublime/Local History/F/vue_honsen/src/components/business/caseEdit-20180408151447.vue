        ...
        <div v-if="curflowobj.casefid" class="caseeditmiddlewrap">
            <div class="middleheadwrap">
                <div class="wrap1150">
                    <div class="middlehead">
                        ...
                        <span v-if="!curflowobj.showcaseinfo && !curflowobj.bigfid">
                            <Dropdown @on-click="createFlow(true)" class="dropdown dropdowneditv2">
                                <Button @click="createFlow(false)" type="info" size="large">添加流程</Button>
                ...
                <div class="wrap1150">
                    ...
                    <div v-if="curflowobj.casefid != -1" :key="editinghash" class="intrustinfo">
                        <div class="commoncaseinfohead caseinfohead">
                            <div>
                                <span v-if="!curflowobj.is_editing" class="tit">{{curflowobj.bigfname ? curflowobj.bigfname + ' > ' + curflowobj.name : curflowobj.name}}</span>
                            ...
                            <div>
                                ...
                                <span v-if="!curflowobj.showcaseinfo && !isEntrust" class="item settime">
                                    <Poptip v-if="curflowobj.is_editing" v-model="show_clock_panel" class="clock_panel_wrap" placement="bottom" width="300" trigger="click" :transfer="false">
                                    ...
                                    <span :style="{color: curflowobj.is_editing ? '#1890ff' : ''}" :key="editingtimehash">{{curflowobj.used_time | timeFormat}}</span>
                                </span>
            ...
            <div class="middlebottom">
                ...
                <div v-if="curflowobj.casefid != -1" class="middlebottomitem middlebottomitem2">
                    <div v-if="!isEntrust" @click="tooglebottomnav('bottomcasekeepnav')" :class="{active: bottomcasekeepnav}">
                        <Badge :dot="!!0"><img src="/static/icon/bizinfo_middlebottom_icon3.png" class="def"><img src="/static/icon/bizinfo_middlebottom_icon3_.png" class="hov"></Badge><span class="num">123</span>
                    </div>
                    <span v-if="!isEntrust" class="line">|</span>
                    <div @click="tooglebottomnav('bottomfilesnav')" :class="{active: bottomfilesnav}">
                        <Badge :dot="!!0"><img src="/static/icon/bizinfo_middlebottom_icon4.png" class="def"><img src="/static/icon/bizinfo_middlebottom_icon4_.png" class="hov"></Badge><span class="num">123</span>
                    </div>
                    <span class="line">|</span>
                    <div v-if="!isEntrust" @click="tooglebottomnav('bottomformnav')" :class="{active: bottomformnav}">
                    ...
                    <span v-if="!isEntrust" class="line">|</span>
                    <div @click="tooglebottomnav('bottomcommentnav')" :class="{active: bottomcommentnav}">
            ...
            <div v-if="bottomcasekeepnav" class="middleconnonnavwrap floatpanel">
                <caseFlowKeep :caseInfo="caseInfo" :curflow="curflowobj" :oRole="oRole"></caseFlowKeep>
            </div>

            <!-- 附件 -->
            <div v-if="bottomfilesnav" class="middleconnonnavwrap floatpanel">
                <caseFlowFiles :caseInfo="caseInfo" :curflow="curflowobj" :oRole="oRole"></caseFlowFiles>
            </div>
            ...
            <div v-if="bottomcommentnav" class="middleconnonnavwrap floatpanel">
                ...
                <div class="conwrap">
                    <div class="pinglun">
                        <comment :id="curflowobj.casefid" type="flow" @changeCommentLength="changeCommentLength($event)"></comment>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="caseeditbottomwrap">
            <div class="caseeditbottomcenter clearfix">
                <div v-for="(flow, index) in aFlows" class="caseeditbottomitemgroup clearfix">
                    <!-- 父流程 -->
                    <div
                        @click="toogleCurflow(flow)"
                        :class="{active: flow.casefid == curflowid}"
                        class="caseeditbottomitem">
                        <p class="tit">{{flow.name}}</p>
                        <div class="con">
                            <span :style="{backgroundColor: index ? bgColorArr[(index-1) % bgColorArr.length] : 'transparent'}" class="topleftcorner"></span><img v-if="index == 0" src="/static/icon/bizinfo_caseindex_thumb.png" class="thumbimg"><img v-else src="/static/icon/bizinfo_caseflow_thumb.png" class="thumbimg">
                        </div>
                    </div>
                    <!-- 子流程 -->
                    <div
                        @click="toogleCurflow(subflow)"
                        v-for="(subflow, subindex) in flow.sub_fids"
                        :class="{active: subflow.casefid == curflowid}"
                        :draggable="true"
                        @dragstart="flowdragstart($event, subflow, subindex, flow)"
                        @dragover.prevent
                        @drop.prevent="flowdrop(subflow, subindex, flow)"
                        class="caseeditbottomitem">
                        <p class="tit">{{subflow.name}}</p>
                        <div class="con">
                            <span :style="{backgroundColor: index ? bgColorArr[(index-1) % bgColorArr.length] : 'transparent'}" class="topleftcorner"></span><img v-if="0" ondragstart="return false;" src="/static/icon/bizinfo_caseindex_thumb.png" class="thumbimg"><img v-else ondragstart="return false;" src="/static/icon/bizinfo_caseflow_thumb.png" class="thumbimg">
                        </div>
                    </div>
                </div>
                ...
                <div class="caseeditbottomitemgroup clearfix">
                    <div class="caseeditbottomitem navaddnewflow">
                        ...
                        <div class="con">
                            ...
                            <div class="btndiv">
                                <Button @click="createFlow(false)" class="btn1" type="info">&nbsp;添加流程&nbsp;</Button>
                                <Button @click="createFlow(true)" v-if="!curflowobj.showcaseinfo && !curflowobj.bigfid" class="btn2" type="ghost">添加子流程</Button>
                            </div>
                        </div>
                    </div>
                </div>
...
import caseFlowKeep from '@/components/business/caseFlowKeep';
import caseFlowFiles from '@/components/business/caseFlowFiles';

export default {
    name: 'caseEdit',
    components: { editor, comment, caseInfo, caseFlowKeep, caseFlowFiles},
    data() {
        return {
            imgUrl,
            caseId: null,
            caseInfo: {},
            bgColorArr: ['#00a0e9', '#9756b7', '#13b5b1'],
            aFlows: [],
            curflowid: -1,
            caseloading: false,
    ...
    computed: {
        ...
        oRole() {
            if (this.caseInfo) {
                ...
                obj.canEditOwnCreated =  (createrid) => {
                    return !this.caseInfo.status && (obj.isOwner || ((obj.isTeam || obj.isCoactor) && (createrid == this.$store.state.mineInfo.id)));
                };
                return obj;
            }
        },
    ...
    methods: {
        ...
        changeCommentLength(obj) {
            this.curflowobj.total = obj.total;
        },
        getCaseInfo() {
        ...
        getFlowList() {
            this.$ajax('get', this.$apiConfig.bizGroup.getCaseFlow.replace(/\{caseid\}/, this.caseInfo.caseid), 'caseloading', data => {
                if( !data.data.length ){
                    data.data.push({
                        casefid: -1,
                        sub_fids: []
                ...
                data.data.forEach(val => {
                    ...
                    val.timer = -1;
                    val.used_time = val.used_time ? val.used_time : 0;
                    val.record_time = val.used_time;
                    val.noticeEditingTimer = -1;
                    val.sub_fids.forEach((element) => {
                        element.bigfid = val.casefid;
                        element.bigfname = val.name;
        ...
        toogleCurflow(flow) {
            if( flow.is_bigflow ){
                ...
                this.$ajax('get', this.$apiConfig.bizGroup.getCaseFlowInfo.replace(/\{caseid\}/, this.caseInfo.caseid).replace(/\{caseffid\}/, flow.casefid).replace(/\{casefid\}/, flow.bigfid), data => {
                    ...
                    data.data[0].timer = -1;
                    data.data[0].used_time = data.data[0].used_time ? data.data[0].used_time : 0;
                    data.data[0].record_time = data.data[0].used_time;
                    data.data[0].noticeEditingTimer = -1;
        ...
        resetAllState() {
            ...
            clearInterval(this.curflowobj.timer);
            clearTimeout(this.curflowobj.noticeEditingTimer);
            this.editinghash +=1;
        },
        ...
        submitFlowContent(type) {
            if( type == 'save' ){
                ...
                this.$ajax('post', `/biz/flow/${this.curflowobj.caseid}/store/${this.curflowobj.casefid}`, param, data => {
                    ...
                    this.curflowobj.record_time = this.curflowobj.used_time;
                    // 重置状态
                    this.curflowobj.is_editing = false;
                    clearInterval(this.curflowobj.timer);
                    clearTimeout(this.curflowobj.noticeEditingTimer);
                ...
                this.curflowobj.used_time = this.curflowobj.record_time;
                // 重置状态
                this.curflowobj.is_editing = false;
                clearInterval(this.curflowobj.timer);
                clearTimeout(this.curflowobj.noticeEditingTimer);
        ...
        openisediting() {
            this.$ajax('get', `/biz/flow/editstatus/${this.curflowobj.casefid}`, data => {
                if (data.is_editing) {
                    ...
                    this.curflowobj.timer = setInterval(() => {
                        this.curflowobj.used_time += 1;
                        this.editingtimehash += 1;
                    }, 1000);
                }
            });
        },
        createFlow(ischild) {
            ...
            if( ischild ){
                console.log(1)
                param.parent_fid = this.curflowobj.casefid;
            }
            this.$ajax('post', this.$apiConfig.bizGroup.addCaseFlow.replace(/\{caseid\}/, this.caseInfo.caseid), param, data => {
                let newFlow = {
                ...
                if( !ischild ){
                    newFlow.is_bigflow = true;
                    this.aFlows.push(newFlow);
                }else{
                    newFlow.bigfid = this.curflowobj.casefid;
                    newFlow.bigfname = this.curflowobj.name;
                    this.curflowobj.sub_fids.push(newFlow);
                }

                this.$Message.success('添加流程成功！');

                this.curflowid = data.casefid;
            });
        },
        noticeFlowEditing() {
            // 通知服务器流程正在编辑中
            this.$ajax('get', this.$apiConfig.bizGroup.setFlowEditing.replace(/\{casefid\}/, this.curflowobj.casefid), (data) => {
                console.log('通知服务器流程正在编辑中的请求成功！');
            });

            // 轮询
            this.curflowobj.noticeEditingTimer = setTimeout(() => {
                this.noticeFlowEditing();
            }, 175000);
        },
        toFlowQuit() {
            if (this.curflowobj.is_editing) {
                this.$ajax('get', this.$apiConfig.bizGroup.setFlowQuitEditing.replace(/\{casefid\}/, this.curflowobj.casefid), (data) => {
                    console.log('流程编辑已退出');
                });
            }
        },
        ...
        flowdragstart(ev, subFlow, index, parentFlow) {
            console.log(ev, subFlow, index, parentFlow)
        ...
        flowdrop(subFlow, index, parentFlow) {
            console.log(subFlow, index, parentFlow)
