            ...
            <Col span="16" class="main">
                <Form :model="form_info" ref="form_info" :rules="form_info_rules" label-position="left" :label-width="95">
                    ...
                    <FormItem label="手机号码：" style="margin-bottom: 19px" prop="mobile">
                        ...
                        <Input v-model.trim="form_info.mobile" :disabled="is_info_auditing" v-onlyNum></Input>
                    </FormItem>
...
export default {
    ...
    methods: {
        ...
        submit_info(name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    if( this.$authCodeT(this.$store.getters.auth_code_123).isLawyer && !this.form_info.mobile ){
                        this.$Message.error('律师必须填写手机号！');
                        return;
                    }
