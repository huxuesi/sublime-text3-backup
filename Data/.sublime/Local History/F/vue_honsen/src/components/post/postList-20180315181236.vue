    ...
    <div class="post_list clearfix">
        ...
        <div class="left_side">
            ...
            <addMember
                ...
                :title="authModalTit"
                :initSelectedUserIds="initUsers"
                @close="authModaltwo = false"
                @selected-users="setPostAuthtwo">
            </addMember>
        </div>
        ...
        <div class="right_side">
            <div class="my_post">
                ...
                <contextMenu ref="cid_contextmenu">
                    ...
                    <div class="contextMenu_item" @click.stop="openAuth('edit', 'group')">编辑授权</div>
                    <div class="contextMenu_item" @click.stop="openAuth('view', 'group')">阅读授权</div>
                </contextMenu>
...
export default {
    ...
    methods: {
        ...
        openAuth(type, isgroup) {
            this.authModalTit = type == 'edit'? '编辑授权': '阅读授权';
            this.$refs.post_contextmenu.hide();
            this.openAuthType = type;
            if( isgroup ){
                this.initUsers = []//this.handleCategory.auth[type];
                this.authModaltwo = true;
            }else{
        ...
        /* 权限(单个设置为覆盖(要传之前有的) */
        setPostAuthtwo(users, ids, isNoChange) {
            if (isNoChange) {
                this.authModaltwo = false;
                return;
            }
            ...
            if (this.openAuthType == 'edit') {
                send_data = {
                    type: 'authEdit',
                    cid: this.handleCategory.cid,
                    data: {"edit": ids, "view": this.handleCategory.auth.view.map(val => val.id)}
                }
            } else {
                send_data = {
                    type: 'authView',
                    cid: this.handleCategory.cid,
                    data: {"edit": this.handleCategory.auth.edit.map(val => val.id), "view": ids}
                }
            }

            this.$ajax('post', `/post/setinfosbyclassify`, param, (data) => {

                this.$set(this.handleCategory.auth, this.openAuthType, users);
