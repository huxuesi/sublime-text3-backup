...
<template>
    <div class="side_collect">
        <sideBase ref="sideBase" :initDragState="true">
            <div slot="side">
            ...
            <div slot="main" style="position: relative">
                <div v-show="side_type == 'post' || side_type == 'tag' || side_type == 'search'" class="main_list">
                    <div
                        ...
                        :id="'post_item' + index"
                        :draggable="true"
                        @dragstart="dragstart($event, post, index)"
                        @dragover.prevent
                        @drop.prevent="drop(post, index)"
                        ...
                        <Row class="main_item main_item_link" :class="{active: current_link && link.id == current_link.id}">
                            <Col span="17">
                                ...
                                <div class="content single_ellipsis">
                                    <a v-if="link.href" :href="link.href.indexOf('http') == -1? ('//'+link.href): link.href" target="_blank">
                                        {{link.href.indexOf('http') == -1? ('//'+link.href): link.href}}
                                    </a>
                                </div>
                            </Col>
                            <Col span="7" class="assistText">
...
export default {
    ...
    methods: {
        ...
        clickPost(post) {
            window.open('/post/read/' + post.pid);
        ...
        dragstart(ev, post, index) {
            ev.dataTransfer.setData('Text', ev.target.id);
            this.dragPost = JSON.parse(JSON.stringify(post));
            this.dragIndex = index;
        },
        drop(post, index) {
            if (post.pid == this.dragPost.pid) {
                return;
            }
            let dropPost = JSON.parse(JSON.stringify(post));

            let param = {
                from: this.dragPost.pid,
                to: post.pid
            };
            this.$ajax('get', '/post/collect/dragcollect', param, (data) => {
                this.posts.splice(this.dragIndex, 1);
                this.posts.splice(index, 0, this.dragPost);
