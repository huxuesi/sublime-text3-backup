...
<template>
    <div class="side_records">
        <sideBase :haveSideNav="true" ref="sideBase">
                    ...
                    <div class="keeprecordsnavlistwrap" v-scrollUnique>
                        <Collapse @on-change="changeKeeprecordsNav" v-model="curKeeprecordsNavId" class="keeprecordsnavlist" accordion>
                            <Panel v-for="list in keeprecords.list" :key="list.id" :name="list.id.toString()">
                                ...
                                <ul slot="content">
                                    <li @click="curKeeprecordsChildId = child.id" v-for="child in list.child" :class="{ active: curKeeprecordsChildId == child.id}" :title="child.name" class="single_ellipsis">
                                        {{child.name}}
                                    </li>
                                </ul>
                            </Panel>
            ...
            <div slot="mainTit">
                <div :style="{ borderBottom: keeprecordisEditing ? '1px solid #dcdcdc' : ''}" class="keeprecordtit"><Icon type="android-add-circle"></Icon></div>
            </div>

            <div slot="main">
                <div class="keeprecordtitcont" v-scrollUnique>
                    <div v-if="!keeprecordisEditing">
                        <div v-for="(list, idx) in curKeeprecords" class="keeprecordtitcontlist">
                    ...
                    <div v-else>
                        <div class="keeprecordEditForm">
                            ...
                            <Input v-model.trim="keeprecordEditForm.content" type="textarea" :rows="14" style="margin-top: 15px"></Input>
                            <div class="keeprecordEditFormsub"><Button type="info" size="large"> 提 交 </Button><Button @click.native="cancelkeeprecordEdit" class="gray_btn" type="info" size="large">取 消</Button></div>
                        </div>
                    </div>
                </div>
            </div>
