...
export default {
    ...
    methods: {
        ...
        dragstart(ev, post, index) {
            console.log(ev, post, index)
            /*ev.dataTransfer.setData('Text', ev.target.id);
            this.dragCategory = JSON.parse(JSON.stringify(post));
            this.dragIndex = index;*/
        },
        drop(post, index) {
            console.log(post, index)
            /*if (post.cid == 0 || post.cid == this.dragCategory.cid) {
                return;
            }
            let dropCategory = JSON.parse(JSON.stringify(post));
