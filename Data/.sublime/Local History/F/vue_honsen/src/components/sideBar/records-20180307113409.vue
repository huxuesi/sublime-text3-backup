...
<template>
    <div class="side_records">
        <sideBase :haveSideNav="true" ref="sideBase" :initDragState="true">
            <div slot="sideNav">
                        ...
                        <span class="selecticon">
                            <Select v-model="selecttemId" @on-change="templateChange" class="selectselectwrap" placeholder="选择模板">
                                <Option v-for="(tpl, tplidx) in TPL" :value="tpl.id" :key="tpl.id">
                                    {{tpl.template_name}}
                                    <div class="del_mod" @click.stop="delTPL(TPL, tplidx, tpl.id)"></div>
                                </Option>
                            </Select>
                        </span>
                    </div>
            ...
            <div slot="main">
                <div v-if="curLeftNav == 'keeprecords' && curKeeprecords" class="keeprecordcont" v-scrollUnique>
                    <div v-if="!keeprecordisEditing">
                        ...
                        <div v-if="!curKeeprecords.length" class="nodatalist">
                            此案件暂无随记
                        </div>
                    </div>
                    <div v-else>
                ...
                <div v-if="curLeftNav == 'keeptasks' && curtasks" class="taskcont" v-scrollUnique>
                    <div v-if="!taskisAdding" :key="taskkey">
                        <div v-for="(list, taskidx) in curtasks" class="taskcontlist">
                            ...
                            <span v-if="!taskAllEditing" @click="deltask(curtasks, taskidx)" class="close">x</span>
                        </div>
                        <div v-if="!curtasks.length" class="nodatalist">
                            此案件暂无任务
                        </div>
    ...
    methods: {
            ...
            this.$ajax('get', '/biz/case/customerforcase', 'keeprecords.listloading', (data) => {
                ...
                v.forEach(e => {
                    e.cases.forEach(i => {
                        ...
                        if( i.tasks && i.tasks.length <= 22000 ){
        ...
        delkeeprecord(l, i, e) {
            this.$Modal.confirm({
                ...
                content: `确定删除吗？`,
                onOk: () => {
                    this.$ajax('get', '/record/delrecord', { id: e.id}, 'keeprecords.listloading', (data) => {
                        l.splice(i, 1);
                    });
                },
                onCancel: () => {
        ...
        delTPL(e, i, id) {
            this.$Modal.confirm({
                ...
                content: `确定删除吗？`,
                onOk: () => {
                    this.$ajax('get', '/task/deltasktemplate', { id: id}, (data) => {
                        e.splice(i, 1);
                    });
                },
                onCancel: () => {
