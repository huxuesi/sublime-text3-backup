...
<template>
    <div class="sideBar animated">
        <div class="icon_list" :style="{ paddingTop: islawyer ? '190px' : '245px'}">
        ...
        <!-- 组件内容 -->
        <keep-alive exclude="feedback,collect,records">
...
export default {
    ...
    data() {
        return {
            icon_list: {
                ...
                record: { tip: '跟进记录', name: 'record', noshow: false},
    ...
    computed: {
        ...
        islawyer() {
            let bol = this.$authCodeT(this.$store.state.mineInfo.auth_code).isLawyer || false;
            // 律师才显示这两项
            this.icon_list.record.noshow = !bol;
            this.icon_list.aide.noshow = !bol;
            return bol;
        }
    },
    beforeMount() {
