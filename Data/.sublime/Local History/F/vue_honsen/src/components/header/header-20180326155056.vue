                ...
                <div class="popup">
                    ...
                    <ul v-if="recentNotifs.length" v-scrollUnique>
                        <li class="clearfix" v-for="notif in recentNotifs">
                            <div class="img">
                                <Badge v-if="notif.type === nType.admin_auth || notif.type === nType.expire_remind || (notif.type === nType.user_evaluate && !notif.detail.f)" :dot="!notif.is_readed">
                            ...
                            <div class="noti_content">
                                ...
                                <!--消息类型-->
                                <span class="text" v-if="notif.type === nType.auth_edit">授予你在<a class="content_link" href="javascript:;" @click="toPostDetail(notif.detail.a)">《{{notif.detail.a_n}}》{{notif.detail.num > 1 ? '等多篇知识' : ''}}</a>中的编辑权限</span>

                                <span class="text" v-if="notif.type === nType.auth_view">授予你在<a class="content_link" href="javascript:;" @click="toPostDetail(notif.detail.a)">《{{notif.detail.a_n}}》{{notif.detail.num > 1 ? '等多篇知识' : ''}}</a>中的阅读权限</span>
                                ...
                                <span class="text" v-if="notif.type === nType.user_evaluate">{{notif.detail.f_n ? '' : '匿名用户 '}}评价了您，可去<a href="javascript:;" @click="$router.push({name: 'personalCenter', params: { id: $store.state.mineInfo.id}})" class="content_link"> 个人中心 </a>进行查看</span>
                            </div>
                        </li>
                    </ul>
                    <div class="read_more">
...
export default {
    ...
    methods: {
        ...
        toPostDetail(id) {
            this.$router.push({name: 'postRead', params: {read_pid: parseInt(id)}});
        },
        toBusinessList(id) {
