        ...
        <div class="newsList-wrapper">
            <ul class="newsList">
                <li class="newsItem" v-for="(notif,index) in notifs" :key="index">
                    ...
                    <Badge v-if="notif.type === nType.admin_auth || notif.type === nType.expire_remind || (notif.type === nType.user_evaluate && !notif.detail.f)" :dot="!notif.is_readed && !isReadNotifs">
                    ...
                    <!--消息类型-->
                    <span class="text" v-if="notif.type === nType.auth_edit">授予你在<a href="javascript:;" @click="toPostDetail(notif.detail.a)">《{{notif.detail.a_n}}》{{notif.detail.num > 1 ? '等多篇知识' : ''}}</a>中的编辑权限</span>

                    <span class="text" v-if="notif.type === nType.auth_view">授予你在<a href="javascript:;" @click="toPostDetail(notif.detail.a)">《{{notif.detail.a_n}}》{{notif.detail.num > 1 ? '等多篇知识' : ''}}</a>中的阅读权限</span>
                    ...
                    <span class="text" v-if="notif.type === nType.user_evaluate">{{notif.detail.f_n ? '' : '匿名用户 '}}评价了您，去<a href="javascript:;" @click="$router.push({name: 'personalCenter', params: { id: $store.state.mineInfo.id}})">个人中心</a>查看</span>
                </li >
