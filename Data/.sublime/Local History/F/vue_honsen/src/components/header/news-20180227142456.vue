        ...
        <div class="newsList-wrapper">
            <ul class="newsList">
                <li class="newsItem" v-for="(notif,index) in notifs" :key="index">
                    ...
                    <span class="text" v-if="notif.type === nType.admin_auth && !notif.detail.result">你的认证审核没有通过，原因：{{notif.detail.reason}}。请重新去 <a href="javascript:;" @click="$router.push({name: 'identitySet'})">认证</a> ！</span>
                    <!-- 日程添加 -->
