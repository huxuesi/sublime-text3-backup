            ...
            <li @click="gotoPage(nType[notif.type], notif.detail.a, notif)" class="clearboth" v-for="notif in notifs">
                ...
                <div class="noti_content">
                    ...
                    <span class="text" v-if="notif.type === nType.auth_edit.type">
                        授予你在&nbsp;《{{notif.detail.a_n}}》{{notif.detail.num > 1 ? '等多篇知识' : ''}}&nbsp;中的编辑权限
                    </span>
                    <span class="text" v-if="notif.type === nType.auth_view.type">
                        授予你在&nbsp;《{{notif.detail.a_n}}》{{notif.detail.num > 1 ? '等多篇知识' : ''}}&nbsp;中的阅读权限
                    </span>
                    ...
                    <span class="text" v-if="notif.type === nType.case_create.type">创建了团队案件《{{notif.detail.a_n}}》</span>
                    ...
                    <span class="text" v-if="notif.type === nType.expire_remind.type">您的账号 {{notif.detail.a_n}}，请及时续费</span>

                    <span class="text" v-if="notif.type === nType.user_evaluate.type">{{notif.detail.f_n ? '' : '匿名用户 '}}评价了您，可去个人中心进行查看</span>
                </div>
            </li>
        </ul>
...
export default {
    ...
    data () {
        return {
            ...
            nType: {
                ...
                case_create: { type: 'case_create', url: 'caseReadKH'},
                ...
                expire_remind: { type: 'expire_remind', url:''},
                user_evaluate: { type: 'user_evaluate', url:'cusComment'},
            },
        }
    },
    computed: {
    ...
    methods: {
        gotoPage(t, id, list) {
            ...
            }else if( t.type == 'user_evaluate' ){
                this.$router.push({ name: t.url, params: { id: this.$store.state.userInfo.id}})
            }else if( t.url ){
