...
<template>
    <div v-infinite-scroll="getschedule"
        ...
        <div v-for="list in schedulelist" class="schedulelist appsmallwrap">
            <div @click="cursid == list.sid ? cursid = '' : cursid = list.sid" :class="{ active: cursid == list.sid}" class="schedulehead"><span class="name singleellipsis">{{list.title}}</span> <span class="arrow"></span></div>
            <div v-if="cursid == list.sid" class="schedulebody">
                <table cellpadding="0" cellspacing="0" class="scheduletable">
                    ...
                    <tr>
                        <td v-if="list.url" style="white-space: nowrap; vertical-align: top">链　　接：</td>
                        <td>{{list.url}}</td>
                    </tr>
                    <tr class="memberstr">
        ...
        getschedule() {
            ...
            this.$http.get(`/schedure/lists`/*, { params: {page: this.schedulelist.length / this.perpage + 1, perpage: this.perpage}}*/).then(response => {
                if( response.data.retcode == RETCODE_OK ){
                    ...
                    v = v.filter(e => moment(e.end).unix() >= moment().unix())
