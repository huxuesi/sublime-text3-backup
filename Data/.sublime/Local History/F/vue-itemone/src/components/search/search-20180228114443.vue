    ...
    <div class="searchresult">
        ...
        <div class="searchrescontent">
            ...
            <div
                ...
                infinite-scroll-distance="30"
                infinite-scroll-immediate-check="false">
                <div v-for="list in biz.list" class="searchbizwrap">
                    <div class="head"><span v-html="list.customer_name"></span><span>{{list.time}}</span></div>
                    <div class="user">客户归属：{{list.user_name}}</div>
                    <div class="case"><span v-html="'案件名称：' + list.case_name" class="casename"></span><span class="state">{{li.case_status == 2 ? '未处理' : li.case_status == 1 ? '已结案' : '处理中'}}</span></div>
                </div>
            </div>
